...
{
    /// <summary>
    /// Contains routines to generate an XML file from an XML object or an ADO.NET DataTable ojbect.
    /// </summary>
    public class XMLFileOutputProcessor
    {
        ...
        private enXMLOutputType _xmlOutputType = enXMLOutputType.DataOnly;
        ...
        public XMLFileOutputProcessor(string outputFilename)
        {
        ...
        public XMLFileOutputProcessor(string outputFilename, bool replaceExistingFile)
        {
            _outputFileName = outputFilename;
        ...
        /// <param name="outputFilename">Full path to the output file.</param>
        /// <param name="replaceExistingFile">True to overwrite existing file with same name. Otherwise, set to false to throw an error if file with same name already exists.</param>
        /// <param name="xmlOutputType">Specify what to include in the XML output: Data only, Data Plus Schema or Schema Only.</param>
        public XMLFileOutputProcessor(string outputFilename, bool replaceExistingFile, enXMLOutputType xmlOutputType)
        {
            _outputFileName = outputFilename;
            _replaceExistingFile = replaceExistingFile;
            _xmlOutputType = xmlOutputType;
        }
        ...
        /// <summary>
        /// Specify what to include in the XML output: Data only, Data Plus Schema or Schema Only.
        /// </summary>
        public enXMLOutputType XMLOutputType
        {
            get
            {
                return _xmlOutputType;
            }
            set
            {
                _xmlOutputType = value;
            }
        }
        ...
        public bool WriteDataToOutput(DataTable dt)
        {
            ...
            try
            {
                ...
                switch (_xmlOutputType)
                {
                    case enXMLOutputType.DataOnly:
                        _exporter.ExportDataTableToXmlFile(dt, _outputFileName);
                        break;
                    case enXMLOutputType.DataPlusSchema:
                        _exporter.ExportDataTableWithSchemaToXmlFile(dt, _outputFileName);
                        break;
                    case enXMLOutputType.SchemaOnly:
                        _exporter.ExportDataTableToXmlSchemaFile(dt, _outputFileName);
                        break;
                    default:
                        _exporter.ExportDataTableToXmlFile(dt, _outputFileName);
                        break;
        ...
        {
            XmlSerializer ser = new XmlSerializer(typeof(XMLFileOutputProcessor));
        ...
        /// <returns>An instance of XMLFileOutputDef.</returns>
        public static XMLFileOutputProcessor LoadFromXmlFile(string filePath)
        {
            XmlSerializer deserializer = new XmlSerializer(typeof(XMLFileOutputProcessor));
            TextReader textReader = new StreamReader(filePath);
            XMLFileOutputProcessor objectInstance;
            objectInstance = (XMLFileOutputProcessor)deserializer.Deserialize(textReader);
            textReader.Close();
        ...
        /// <returns>An instance of XMLFileOutputDef.</returns>
        public static XMLFileOutputProcessor LoadFromXmlString(string xmlString)
        {
            XmlSerializer deserializer = new XmlSerializer(typeof(XMLFileOutputProcessor));
            StringReader strReader = new StringReader(xmlString);
            XMLFileOutputProcessor objectInstance;
            objectInstance = (XMLFileOutputProcessor)deserializer.Deserialize(strReader);
            strReader.Close();
            return objectInstance;
        }
