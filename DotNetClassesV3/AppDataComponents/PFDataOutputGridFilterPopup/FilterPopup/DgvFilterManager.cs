...
/*
Code for the PFDataGridViewFilterPopup class was downloaded from CodeProject application developed by Vincenzo Rossi.

http://www.codeproject.com/Articles/33786/DataGridView-Filter-Popup

Some minor modifications were made to accomodate ProFast Computing software requirements.
*/

namespace PFDataGridViewFilterPopup 
{
    ...
    public class DgvFilterManager {
        ...
        private bool _suppressFilterRebuild = false;
        ...
        {
            //change profast
            //do not rebuild filter if the RebuildColumnFilterLists method is running. See bottom of this document for that method.
            ...
            foreach (DgvBaseColumnFilter CF in mColumnFilterList) 
            {
                ...
                else 
                {
                    ...
                    CF.FilterExpression = ""; //fix by ProFast Computing July/2014
                }
            ...
            try 
            {
                ...
                {
                    if (mBindingSource.Filter != Filter)
                    {
                        mBindingSource.Filter = Filter;
                        RebuildColumnFilterLists(Filter);
                    }
                } 
                else 
                {
                    if (mBoundDataView.RowFilter != Filter)
                    {
        ...
        /// <summary>
        /// Rebuilds the drop-down lists for combobox column filters when either the bindingsource or dataview filter has changed.
        /// </summary>
        /// <param name="filter">New filter.</param>
        public void RebuildColumnFilterLists(string filter)
        {
            ...
            try
            {
                ...
                foreach (DgvBaseColumnFilter bcf in mColumnFilterList)
                {
                    if (bcf is DgvComboBoxColumnFilter)
                    {
                        bool saveActiveFlag = bcf.Active;
                        DgvComboBoxColumnFilter cbf = (DgvComboBoxColumnFilter)bcf;
                        cbf.RefreshValuesWithFilter(filter);
                        bcf.Active = saveActiveFlag;
                    }
                }
            }
