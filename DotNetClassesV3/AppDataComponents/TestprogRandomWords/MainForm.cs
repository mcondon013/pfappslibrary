...
namespace TestprogRandomWords
{
    public partial class MainForm : Form
    {
        ...
        private void SetFormValues()
        {
            ...
            configValue = AppGlobals.AppConfig.GetStringValueFromConfigFile("DefaultRandomDataDatabase", string.Empty);
            if (configValue.Length > 0)
                this.txtRandomDataDatabase.Text = configValue;
            else
                this.txtRandomDataDatabase.Text = _defaultRandomDataDatabase;

            configValue = AppGlobals.AppConfig.GetStringValueFromConfigFile("DefaultRandomDataDatabasePassword", string.Empty);
            if (configValue.Length > 0)
                this.txtRandomDataDatabasePassword.Text = configValue;
            else
                this.txtRandomDataDatabasePassword.Text = _defaultRandomDataDatabasePassword;

            configValue = AppGlobals.AppConfig.GetStringValueFromConfigFile("DefaultRandomDataXmlFilesFolder", string.Empty);
            if (configValue.Length > 0)
                this.txtRandomDataXmlFilesFolder.Text = configValue;
            else
                this.txtRandomDataXmlFilesFolder.Text = _defaultRandomDataXmlFilesFolder;

            string[] fileNames = Directory.GetFiles(this.txtRandomDataXmlFilesFolder.Text, "*.xml");
            this.cboRandomDataXmlFile.Items.Clear();
            foreach (string filename in fileNames)
            {
                string filenameonly = Path.GetFileName(filename);
                //ignore following files: they are in a format that contains more than just the word in the data
                //current test routine for outputting words can only handle files that only contain word and no other data
                if (filenameonly.Equals("FirstNames", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("FirstNamesEN", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("FirstNamesFR", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("FirstNamesSP", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("CMSA_Codes", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("MSA_Codes", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("Area_Codes_Canada", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("ProvinceCodesCanada", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("StateCodesMexico", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("StateCodesUS", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("tblUS_ZipCityStats", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("tblCanada_PostalCodeCityStats", StringComparison.OrdinalIgnoreCase) == false
                    && filenameonly.Equals("tblMexico_PostalCodeCityStats", StringComparison.OrdinalIgnoreCase) == false)
                {
                    this.cboRandomDataXmlFile.Items.Add(filenameonly);
                }
            }

            this.txtNumWordsToOutput.Text = "100";
            this.txtNumSentencesToOutput.Text = "10";
            this.txtNumParagraphsToOutput.Text = "3";
            this.txtNumChaptersToOutput.Text = "3";
        }
        ...
        private void InitAppProcessor()
        {
            ...
            _appProcessor.HelpFilePath = _helpFilePath;
            _appProcessor.MainInputForm = this;
        }
        ...
        private void RunTests()
        {
            ...
            try
            {
                ...
                if (this.chkGenerateRandomWords.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomWords();
                }

                if (this.chkGenerateSampleRandomSentences.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateSampleRandomSentences();
                }

                if (this.chkGenerateRandomSentences.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomSentences();
                }

                if (this.chkGenerateRandomParagraphs.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomParagraphs();
                }

                if (this.chkGenerateRandomDocument.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomDocument();
                }

                if (this.chkGenerateRandomChapter.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomChapter();
                }

                if (this.chkGenerateRandomBook.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateRandomBook();
                }

                if (this.chkGenerateSentenceTemplateArrays.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.GenerateSentenceTemplateArrays();
                }
