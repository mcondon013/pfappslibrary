...
namespace TestprogAppDataForms
{
    public partial class MainForm : Form
    {
        ...
        {
            this.cboTableName.Text = "DimCustomer";
        }
        ...
        private void RunTests()
        {
            ...
            try
            {
                ...
                if (this.chkShowFixedLenInputColDefsForm.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.ShowFixedLenInputColSpecForm(this.cboTableName.Text);
                }

                if (this.chkShowFilterBuilderForm.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.ShowFilterBuilderForm(this.cboTableName.Text);
                }

                if (this.chkTestRowFilter.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.TestRowFilter();
                }

                if (this.chkShowFilterForm.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.ShowFilterForm(this.cboTableName.Text, this.cboFilterFile.Text);
                }

                if (this.chkShowFixedLenOutputColDefsForm.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.ShowFixedLenOutputColSpecForm(this.cboTableName.Text);
                }

                if (this.chkShowFxlVisualParseForm.Checked)
                {
                    nNumTestsSelected++;
                    _appProcessor.ShowFxlVisualParseForm();
                }
