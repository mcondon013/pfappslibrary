...
namespace pfAppConfigManager
{
    public partial class ApplicationOptionsForm : Form
    {
        ...
        private void RestoreOriginalAppSettings()
        {
            ...
            string appName = AppInfo.AssemblyProduct;
            string origAppSettingsPath = string.Empty;
            ...
            try
            {
                ...
                if(String.IsNullOrEmpty(Path.GetDirectoryName(this.OrigSettingsFileName)))
                {
                    origAppSettingsPath = Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase), this.OrigSettingsFileName).Replace(@"file:\","");
                }
                else
                {
                    origAppSettingsPath = this.OrigSettingsFileName;
                }

                //fileName = Path.Combine(Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase), this.OrigSettingsFileName).Replace(@"file:\","");
                fileName = origAppSettingsPath;
